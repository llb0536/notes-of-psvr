class OFPSVR.models.Reply extends Backbone.Model
  ...
  constructor:(options)->
    super(options)
    @urlRoot = '/notes/'+this.get('noteId')+'/replies'
    
  validate: (attrs)->
    unless attrs.suc
      unless attrs.username.length>=2 and attrs.username.length <= 15
        return 'User name should be of length 2..10'
      unless @emailregexp.test(attrs.email)
        return 'Email address is not valid'
      unless attrs.website.length <= 200
        return 'Website address is tooooooo long(What are you doing?)'
      unless attrs.body.length <= 300 and attrs.body.length >=1
